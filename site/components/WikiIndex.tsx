...
export default function WikiIndex({ entries }: { entries: WikiEntry[] }) {
  ...
  return (
    <div>
      {/* Tabs */}
      <div className="flex overflow-x-auto scrollbar-hide border-b border-[var(--border)] mb-5 -mx-6 px-6">
        ...
        <div className="flex flex-col gap-1.5">
          {filtered.map((entry) => (
            <Link
              ...
              className="bg-[var(--card)] border border-[var(--border)] rounded-xl px-5 py-3.5 text-sm font-medium hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors"
            >
              {entry.title}
            </Link>
          ))}
